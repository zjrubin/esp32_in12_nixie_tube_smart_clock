...
TaskHandle_t g_task_nixie_saver_mode_handle = NULL;
...
void task_nixie_saver_mode(void* pvParameters);
...
void task_nixie_saver_mode(void* pvParameters) {
  for (;;) {
    ...
    if (xSemaphoreTake(Nixie_Display::display_mutex, portMAX_DELAY) == pdTRUE) {
      ...
      // uint8_t eeprom_special_mode =
      // EEPROM.read(EEPROM_SPECIAL_MODES_ADDRESS);

      // Set the config value back to zero to resume normal clock operation
      // Once the special mode is done
      // EEPROM.write(EEPROM_NIXIE_SAVER_MODE_ADDRESS, 0);
      // EEPROM.commit();
